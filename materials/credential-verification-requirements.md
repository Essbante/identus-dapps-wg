...
- The issuer,
- The subject (the id property inside the `credentialSubject`),
- The type of the credential,
- The issuance date and optionally the expiration date,
- The claims (i.e., the actual data that the credential is about. Also in the `credentialSubject`).
...
Furthermore, the JWT specification allows for a wide range of different algorithms to sign the token (in this case, the credential), but Identus supports only the ES256K (secp256k1 + SHA256) and EdDSA (Ed25519) algorithms.
This means that Identus, in its current state, cannot verify the validity of every Credential allowed by the spec, but only a very specific subset. This isn't unusual in the world of SSI, but it is important to note, since potential limitations in the verification within a dApp might be circumvented by using either a different proof mechanism (Linked Data Integrity Proofs) or a different signature algorithm. Both can theoretically be easily implemented into Identus if the need arises.
...
The different parts of the JWT are then run through a verification function provided by a cryptographic library, which returns true or false.
...
The next step is to check the `issuanceDate` and the `expirationDate`. Every credential has a field for the issuance date. In most cases, this issuance date is indeed the date of issuance, but it can also be in the future. This is a bit confusing; this was renamed to `validFrom` in the 2.0 spec.
...
Identus implements the [StatusList2021 specification](https://www.w3.org/TR/2023/WD-vc-status-list-20230427/).
...
- A Credential Subject is optional (not in Identus)
- A Credential can have more than one subject (but only one issuer)
- There is no size limitation of credentials. With embedded images (e.g. base64), Credentials can get quite large (theoretically)
- Signature Verification is not enough, the trust-chain is based on the Resolver, a registry (or predefined party), as well as a governance framework on what is allowed
- ...
...
- Use the components of Identus independently, such as specific modules or libraries provided by Identus e.g. `Apollo` for cryptography and `Pollux` for Verifiable Credentials.
...
- Build a custom solution from scratch which performs the steps layout out above manually (but of course not the cryptographic parts).

But in any case, DID resolution has to be done if one uses `did:prism` (not long form). Here we have the following options:
...
## Challanges and open questions for doing verification inside a smart contract
- Can the signature verification for Ed25519 or secp256k1 be performed
- DID-Resolution for the issuer requires an orcale, or
  - A Long-form `did:prism`
  - `did:key`, `did:peer` or some other method
- Revocation would need an orcale (if needed)
- TrustRegistries would need an orcale (if needed)
